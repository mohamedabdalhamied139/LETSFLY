...
class TableVerseApp(QMainWindow):
    ...
    def _handle_ws_event(self, event: dict):
        ...
        if et == "game_action_result":
            ...
            if generation != self._room_generation or str((self.current_room or {}).get("id")) != room_id:
                return
            if event.get("ok"):
                if on_success:
        ...
        if et == "ws_connected":
            # WS carries change notifications, while game state is personalized
            # per user. Keep the reconciliation poll alive and force an immediate
            # state fetch so a missed/early WS notification can never leave the
            ...
            if self.current_room:
                self.poll_timer.start()
                QTimer.singleShot(0, self._poll_table_state)
            return
        ...
        if et == "voice_joined":
            self.voice.handle_server_event(event)
            return
        ...
        if et == "activity_event":
            ...
            # delivery and the live activity delivery cannot add the same event twice.
            if str(event.get("category", "")).upper() == "GAMEPLAY" and event.get("game_event_id") is not None:
                ...
                seen = getattr(self, "_seen_game_activity_events", None)
                if seen is None:
                    seen = set()
                    self._seen_game_activity_events = seen
                seen.add(key)
                if len(seen) > 512:
        ...
        event_room_id = event.get("room_id") if isinstance(event, dict) else None
        if event_room_id is not None and str(event_room_id) != str(self.current_room.get("id")):
            return

        if et == "room_snapshot":
            room = event.get("room") or {}
            if room.get("id") == self.current_room.get("id"):
                self._recover_room_snapshot(
                    room,
                    event.get("uno_state"),
            ...
            if et == "player_connection_lost":
                ...
                if self.current_room:
                    ...
                    if uid is not None:
                        try:
                            uid = int(uid)
                            if uid not in self.current_room.setdefault("players", []):
                                self.current_room["players"].append(uid)
                            pdict = self.current_room.setdefault("players_dict", {})
                            pdict[str(uid)] = name
                        except Exception:
                            pass
        ...
        if et == "chat_message" or "text" in (event or {}):
            ...
            text = event.get("text", "")
            if text:
                self.table_view.add_log(f"{sender}: {text}", category="TABLE_CHAT")
                focused = getattr(self.table_view, "is_chat_focused", lambda: False)()
                if not focused:
        ...
        if et == "score_updated":
            ...
            self.table_view.add_log(f"{name} {total}")
            return

        if et in ("tennis_action_result", "tennis_sound", "tennis_point_result"):
            if hasattr(self.table_view, "tennis_game"):
                self.table_view.tennis_game.handle_event(event)
            return

        if et == "tennis_state_changed":
            state = event.get("state")
            if state:
                self._apply_tennis_state(state)
            else:
                self._poll_table_state()
            return
        ...
        if et == "farkle_match_finished":
            ...
            self.table_view.set_playing_mode(False)
            if hasattr(self.table_view, "farkle_dice_list") and self.table_view.farkle_dice_list:
                self.table_view.farkle_dice_list.clear()
            self.table_view.main_table_widget.setFocus()
            return
        ...
        # State updates dispatching
        if et in ("uno_state_changed", "game_state_changed", "ninety_nine_state_changed", "thief_state_changed", "farkle_state_changed", "domino_state_changed", "american_domino_state_changed", "snakes_state_changed", "scopa_state_changed"):
            state = event.get("state")
            game = self.current_room.get("game", "")
            if state:
                from client.table_framework.adapter import get_adapter
                adapter = get_adapter(game)
                if adapter and adapter.apply_state:
                    adapter.apply_state(self, state)
                elif game == "SCOPA": self._apply_scopa_state(state)
                elif game == "TENNIS": self._apply_tennis_state(state)
                elif game == "THIEF_HUNT": self._apply_thief_state(state)
                elif game == "FARKLE": self._apply_farkle_state(state)
                elif game in ("DOMINO", "AMERICAN_DOMINO"): self._apply_domino_state(state)
                elif game == "SNAKES_LADDERS": self._apply_snakes_state(state)
                elif game == "NINETY_NINE": self._apply_ninety_nine_state(state)
                else: self._apply_uno_state(state)
            else:
                # Always reconcile a state-change notification. This endpoint
                # returns the viewer's private hand/dice/cards, so it cannot be
