...
/// Sound service providing 100% parity with the Windows desktop client SoundEngine.
/// Plays game sounds naturally, supports 3D stereo panning, and connects turn vibration.
class SoundService {
  ...
  double _volume = 1.0;
  bool _isMuted = false;
  // Audio ducking toggle - preserved for future game settings while keeping sounds natural as in Windows
  bool audioDuckingEnabled = false;
  ...
  /// If it's TURN_START and isMyTurn is true, triggers tactile haptic feedback.
  Future<void> playSound(String cueOrPath, {bool isMyTurn = false}) async {
    final cueUpper = cueOrPath.toUpperCase();

    // Trigger haptic only when it's the player's turn per explicit instruction
    if (cueUpper == 'TURN_START' && isMyTurn) {
      await HapticService.instance.playTurnHaptic();
    }
  ...
  /// Play sound with 3D/stereo balance (-1.0 left, 0.0 center, 1.0 right) for Tennis & spatial table seats
