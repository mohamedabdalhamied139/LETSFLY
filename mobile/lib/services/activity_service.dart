...
/// Implements strict deduplication matching Windows desktop client architecture.
/// Satisfies user rule: Categories with 0 events are dynamically hidden from selection tabs.
class ActivityLogService extends ChangeNotifier {
  ...
  static const Map<String, String> logOpenedAnnouncements = {
    'ar': 'تم عرض سجل الأحداث',
    'en': 'Activity log opened',
    'fr': 'Journal d\'activité affiché',
  };
  ...
  /// Returns dynamically filtered categories that currently contain at least one event.
  /// Per user rule: If no events exist in a category (e.g. FRIEND_REQUESTS), it is hidden.
  List<String> get visibleCategories {
    final Set<String> activeCategories = {'ALL'};
    for (final event in _events) {
      final cat = _category(event);
      if (categories.contains(cat)) {
        activeCategories.add(cat);
      }
    }
    // Maintain canonical ordering for consistent navigation
    return categories.where((c) => activeCategories.contains(c)).toList();
  }
  ...
  List<Map<String, dynamic>> get filteredEvents {
    final active = visibleCategories;
    // If selected category has no events and is not visible, fallback to ALL
    final effectiveCategory = active.contains(_selectedCategory) ? _selectedCategory : 'ALL';
    if (effectiveCategory == 'ALL') {
      return List.unmodifiable(_events);
    }
    return List.unmodifiable(
      _events.where((e) => _category(e) == effectiveCategory).toList(),
    );
  }

  /// Announces to TalkBack and VoiceOver that the Activity Log is displayed.
  /// Formatted in the 3 supported languages (AR, EN, FR) according to current active language.
  Future<void> announceLogOpened() async {
    final lang = LocalizationService.instance.currentLanguage.toLowerCase();
    final message = logOpenedAnnouncements[lang] ?? logOpenedAnnouncements['ar']!;
    await AccessibilityManager.instance.announce(message);
  }
